...
public abstract class Actor {
  ...
  // FIXME docs
  static public <T> Behavior<T> mutable(akka.japi.function.Function<ActorContext<T>, MutableBehaviorHasOnMessage<T>> producer) {
    return deferred(ctx -> (MutableBehavior<T>) producer.apply(ctx));
  }

  public static interface MutableBehaviorHasOnMessage<T> {
    Behavior<T> onMessage(T msg) throws Exception;
  }
  
  //FIXME docs
  public static abstract class MutableBehavior<T> extends ExtensibleBehavior<T> implements MutableBehaviorHasOnMessage<T>{
    @Override
    ...
    final Behavior<T> onSignal(Signal msg) throws Exception {
      return Actor.same();
    }
  }
