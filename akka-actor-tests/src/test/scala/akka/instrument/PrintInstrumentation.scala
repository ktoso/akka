 ...
 */
class PrintInstrumentation(dynamicAccess: DynamicAccess, config: Config) extends Instrumentation {

  private val delegate: Instrumentation = {
    val instrumentation = Try { config.getString("akka.print-instrumentation.delegate") } getOrElse "akka.instrument.NoInstrumentation"
    Instrumentation.create(instrumentation, dynamicAccess, config)
  }
  ...
  override def actorCompleted(actorRef: ActorRef, message: Any, sender: ActorRef, context: AnyRef): Unit = {
    print(s"actor completed: $actorRef ! $message (sender = $sender, context = $context)")
    delegate.actorCompleted(actorRef, message, sender, context)
  }
