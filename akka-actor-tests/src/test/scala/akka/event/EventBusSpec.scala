...
abstract class EventBusSpec(val busName: String, conf: Config = AkkaSpec.testConf) extends AkkaSpec(conf) with BeforeAndAfterEach {
...
class ActorEventBusSpec(conf: Config) extends EventBusSpec("ActorEventBus", conf) {
  ...
  "must allow multiple ActorEventBus instances, and shutting them down independently" in {
    // this actually tests if the started unsubscriber actors get unique names, thus allowing multiple instances of buses
    val bus2 = createNewEventBus()
    val otherUnsubscriber = bus2.unsubscriber

    watch(otherUnsubscriber)
    bus2.shutdown()
    expectTerminated(otherUnsubscriber)
    unwatch(otherUnsubscriber)
  }

  "must shutdown underlying unsubscriber actor" in {
    val unsubscriber = bus.unsubscriber

    watch(unsubscriber)
    bus.shutdown()
    expectTerminated(unsubscriber)
  }
