...
private[persistence] trait Eventsourced extends Processor {
  // todo consolidate these traits as PersistentActor
  ...
  private val processingCommands: State = new State {
    ...
    private def onWriteComplete(): Unit = {
      persistInvocations.remove(0) // drop / tail do not mutate the ListBuffer

      val nextIsStashing = persistInvocations.nonEmpty && persistInvocations.head.isInstanceOf[StashingPersistInvocation]
  ...
  private val persistingEvents: State = new State {
    ...
    def aroundReceive(receive: Receive, message: Any) = message match {
      ...
      case p: PersistentRepr ⇒
        ...
        throw new UnsupportedOperationException("Persistent commands not supported")
      case WriteMessageSuccess(p) ⇒
        val invocation = persistInvocations.head
      ...
      case e @ WriteMessageFailure(p, _) ⇒
        ...
        onWriteComplete(persistInvocations.head)
      case s @ WriteMessagesSuccess ⇒ Eventsourced.super.aroundReceive(receive, s)
      case f: WriteMessagesFailure  ⇒ Eventsourced.super.aroundReceive(receive, f)
  ...
  private val persistInvocations = mutable.ListBuffer[PersistInvocation]()
  ...
  final def persist[A](event: A)(handler: A ⇒ Unit): Unit = {
    pendingStashingPersistInvocations += 1
    persistInvocations append StashingPersistInvocation(event, handler.asInstanceOf[Any ⇒ Unit])
  ...
  final def persistAsync[A](event: A)(handler: A ⇒ Unit): Unit = {
    persistInvocations append AsyncPersistInvocation(event, handler.asInstanceOf[Any ⇒ Unit])
    persistentEventBatch = PersistentRepr(event) :: persistentEventBatch
  }
 ...
 */
// todo remove EventsourcedProcessor / Processor
 ...
 * Java API: an persistent actor - can be used to implement command or event sourcing.
 */
abstract class AsbtractPersistentActor extends AbstractEventsourcedProcessor
