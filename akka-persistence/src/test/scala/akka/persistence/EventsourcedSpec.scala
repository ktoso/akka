...
abstract class EventsourcedSpec(config: Config) extends AkkaSpec(config) with PersistenceSpec with ImplicitSender {
  ...
  "An eventsourced processor" must {
    ...
    "support a mix of persist calls (sync, async) and persist calls" in {
      ...
      processor ! Cmd("c")
      info("received:" + expectMsg("a"))
      info("received:" + expectMsg("a-e1-1")) // persist, must be before next command

      info("received:" + expectMsgAnyOf("b", "a-ea2-2")) // ea2 is persistAsync, b (command) can processed before it
      info("received:" + expectMsgAnyOf("b", "a-ea2-2")) // -- || --
      info("received:" + expectMsg("b-e1-3")) // persist, must be before next command

      info("received:" + expectMsgAnyOf("c", "b-ea2-4")) // ea2 is persistAsync, b (command) can processed before it
      info("received:" + expectMsgAnyOf("c", "b-ea2-4")) // -- || --
