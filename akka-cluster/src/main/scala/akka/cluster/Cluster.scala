...
class Cluster(val system: ExtendedActorSystem) extends Extension {
  ...
  /** Data center to which this node belongs to (defaults to "default" if not set explicitly) */
  ...
  val crossDcFailureDetector: FailureDetectorRegistry[Address] = {
    val createFailureDetector = () ⇒
      FailureDetectorLoader.load(settings.CrossDcFailureDetectorSettings.ImplementationClass, settings.CrossDcFailureDetectorSettings.multiDcConfig, system)
