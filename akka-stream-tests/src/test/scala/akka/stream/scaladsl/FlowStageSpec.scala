...
class FlowStageSpec extends AkkaSpec(ConfigFactory.parseString("akka.actor.debug.receive=off\nakka.loglevel=INFO")) {
  ...
  "A Flow with transform operations" must {
    ...
    //      val p = TestProbe()
    //
    //      val ref = Source.actorRef[Int](2, OverflowStrategy.fail)
    //        .transform(() ⇒
    //          new PushStage[Int, Int] {
    ...
    //        .to(Sink.ignore).run()
    //
    //      p.expectMsg("preStart: unknown-operation") // TODO not valid, needs to be "example-stage", once https://github.com/akka/akka/issues/15707 fixed
    //
    //      ref ! 1
    //      p.expectMsg("elem: 1")
    //
    ...
    //
    //      ref ! PoisonPill
    //      p.expectMsg("postStop: unknown-operation") // TODO not valid, needs to be "example-stage", once https://github.com/akka/akka/issues/15707 fixed
    //
    //    }
