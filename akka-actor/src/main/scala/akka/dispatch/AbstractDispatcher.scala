...
object Envelope {
  def apply(message: Any, sender: ActorRef, system: ActorSystem): Envelope =
    apply(message, sender, system, Instrumentation.EmptyContext)
  ...
  def apply(message: Any, sender: ActorRef, system: ActorSystem, context: AnyRef): Envelope = {
    ...
    if (context eq Instrumentation.EmptyContext) Envelope(message, sndr) else new InstrumentedEnvelope(message, sndr, context)
  }
