...
class ConfigChecker(system: ExtendedActorSystem, config: Config, reference: Config) {
  ...
  private def checkDefaultDispatcherSize(): List[ConfigWarning] =
    ifEnabled("default-dispatcher-size") { checkerKey ⇒
      ...
      else if (size < 5 && config.getString("akka.actor.provider") == "akka.cluster.ClusterActorRefProvider")
        warn(checkerKey, path, // TODO remove this check when using Akka 2.4.x
          "Don't configure default-dispatcher with less than 5 threads, because otherwise there is a risk of " +
            "deadlock when initializing the Cluster extension. This limitation does not exist in Akka 2.4.x.")
      else if (size <= 3)
        warn(checkerKey, path,
  ...
  private def checkAutoDown(): List[ConfigWarning] =
    ifEnabled("auto-down") { checkerKey ⇒
      val (path, enabled) = isAutoDownEnabled
  ...
  private def isAutoDownEnabled: (String, Boolean) = {
    val path1 = "akka.cluster.auto-down-unreachable-after"
    val path2 = "akka.cluster.auto-down" // deprecated in 2.3
    if (config.getString(path1).toLowerCase(Locale.ROOT) != "off") (path1, true)
    else if (config.getBoolean(path2)) (path2, true)
    else (path1, false)
  }
    ...
    ifEnabled("cluster-dispatcher") { checkerKey ⇒
      ...
      if (clusterDispatcher != "" && clusterDispatcher != defaultDispatcherPath) {
        if (config.hasPath(clusterDispatcher)) {
          ...
          val w2 = if (size < 5)
            warn(checkerKey, List(path, clusterDispatcher), // TODO remove this check when using Akka 2.4.x, check for size < 2 instead
              "Don't configure Cluster dispatcher with less than 5 threads, because otherwise there is a risk of " +
                "deadlock when initializing the Cluster extension. This limitation does not exist in Akka 2.4.x.")
          else Nil
          List(w1, w2).flatten
  ...
  private def checkSplitBrainResolver(): List[ConfigWarning] =
    ifEnabled("split-brain-resolver") { checkerKey ⇒
      ...
      val downRemoval = config.getDuration(downRemovalPath, MILLISECONDS).millis
      ...
      val (autoDownPath, autoDownEnabled) = isAutoDownEnabled
      val sbrStrategyPath = "akka.cluster.split-brain-resolver.active-strategy"
      val w4 = if (autoDownEnabled && config.getString(sbrStrategyPath).toLowerCase(Locale.ROOT) != "off")
