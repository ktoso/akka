...
class CsvStreamingSpec extends WordSpec with Matchers with ScalatestRouteTest
  ...
  implicit val fct =
    FramingWithContentType(framing, ContentTypes.`text/csv(UTF-8)`)
  // TODO change such that we can support csv and json in one file?
  ...
  val PostCsv = (path: String, data: String) ⇒
    Post(path).withEntity(ContentTypes.`text/csv(UTF-8)`, data)
  ...
  "Example CsvStreaming" should {
    ...
    "reject POST with Content-Type `application/json`" in {
      ...
      PostJson("/", lineByLine) ~> route ~> check {
        ...
        rejection shouldEqual UnsupportedRequestContentTypeRejection(Set(ContentTypes.`text/csv(UTF-8)`))
      }
    }
  }
}