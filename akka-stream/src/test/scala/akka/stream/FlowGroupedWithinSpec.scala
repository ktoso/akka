...
class FlowGroupedWithinSpec extends AkkaSpec with ScriptedTest {
  ...
  "A GroupedWithin" must {
    ...
    //      val p = StreamTestKit.PublisherProbe[Long]()
    //      val c = StreamTestKit.SubscriberProbe[immutable.Seq[Long]]()
    //      Flow(p).groupedWithin(1000, 1.second).produceTo(materializer, c)
    //      val pSub = p.expectSubscription
    //      val cSub = c.expectSubscription
    //      cSub.request(100)
    //      val demand1 = pSub.expectRequest
    //      (1L to demand1) foreach { _ ⇒ pSub.sendNext(input.next()) }
    //      val demand2 = pSub.expectRequest
    //      (1L to demand2) foreach { _ ⇒ pSub.sendNext(input.next()) }
    //      val demand3 = pSub.expectRequest
    //      c.expectNext((1L to (demand1 + demand2)).toVector)
    //      (1L to demand3) foreach { _ ⇒ pSub.sendNext(input.next()) }
    //      c.expectNoMsg(300.millis)
    //      c.expectNext(((demand1 + demand2 + 1L) to (demand1 + demand2 + demand3)).toVector)
    //      c.expectNoMsg(300.millis)
    //      pSub.expectRequest
    //      val last: Long = input.next()
    //      pSub.sendNext(last)
    //      pSub.sendComplete()
    ...
    "buffer groups until requested from downstream" in {
      ...
      val demand1 = pSub.expectRequest
      (1L to demand1) foreach { _ ⇒ pSub.sendNext(input.next()) }
      c.expectNext((1L to demand1).toVector)
      val demand2 = pSub.expectRequest
      (1L to demand2) foreach { _ ⇒ pSub.sendNext(input.next()) }
      c.expectNoMsg(300.millis)
      cSub.request(1)
