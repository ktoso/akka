...
private[akka] class OneBoundedInterpreter(ops: Seq[Stage[_, _]],
  ...
  private sealed trait State extends DetachedContext[Any] with LifecycleContext with BoundaryContext with AsyncContext[Any, Any] {
  ...
  /** Exposes the bare minimum context needed for lifecycle callbacks - can be cached */
  private val LifecycleContext = new LifecycleContext {
    override def materializer: FlowMaterializer = OneBoundedInterpreter.this.materializer
    override def attributes: OperationAttributes = OneBoundedInterpreter.this.attributes
  }
