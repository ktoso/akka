...
class HttpEntitySpec extends FreeSpec with MustMatchers with BeforeAndAfterAll {
  ...
  "HttpEntity" - {
    "support dataBytes" - {
      ...
      "Default" in {
        Default(tpe, 11, producer(abc, de, fgh, ijk)) must
          collectBytesTo(abc, de, fgh, ijk)
      }
      "CloseDelimited" in {
        CloseDelimited(tpe, producer(abc, de, fgh, ijk)) must
          collectBytesTo(abc, de, fgh, ijk)
      }
      "Chunked w/o LastChunk" in {
        Chunked(tpe, producer(Chunk(abc), Chunk(fgh), Chunk(ijk))) must
          collectBytesTo(abc, fgh, ijk)
      }
      "Chunked with LastChunk" in {
        Chunked(tpe, producer(Chunk(abc), Chunk(fgh), Chunk(ijk), LastChunk)) must
          collectBytesTo(abc, fgh, ijk)
      }
    }
    "support toStrict" - {
