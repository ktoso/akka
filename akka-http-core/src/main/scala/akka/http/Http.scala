...
object Http extends ExtensionKey[HttpExt] {
  ...
  final case class IncomingConnection(remoteAddress: InetSocketAddress,
                                      requestProducer: Publisher[HttpRequest],
                                      responseConsumer: Subscriber[HttpResponse]) extends model.japi.IncomingConnection {
    /** Java API */
    def getRequestProducer: Publisher[japi.HttpRequest] = requestProducer.asInstanceOf[Publisher[japi.HttpRequest]]
    /** Java API */
    def getResponseConsumer: Subscriber[japi.HttpResponse] = responseConsumer.asInstanceOf[Subscriber[japi.HttpResponse]]
  }
