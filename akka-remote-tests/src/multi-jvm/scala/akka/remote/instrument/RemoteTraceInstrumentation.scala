...
import akka.instrument.{ Instrumentation, Trace }
...
class RemoteTraceInstrumentation(config: Config) extends EmptyRemoteInstrumentation {
  ...
  private def appendContext(actorRef: ActorRef): AnyRef = getContext(actorRef) match {
    ...
    case _            ⇒ Instrumentation.EmptyContext
  }

  private def getContext(actorRef: ActorRef): AnyRef =
    if (actorRef.path.parent.name == "user") Trace.value else Instrumentation.EmptyContext

  private def setContext(context: AnyRef): Unit =
    if (context ne Instrumentation.EmptyContext) Trace.value = context
  ...
  private def deserializeContext(context: Array[Byte]): AnyRef =
    SerializationExtension(system).deserialize(context, classOf[String]).getOrElse(Instrumentation.EmptyContext)
}
