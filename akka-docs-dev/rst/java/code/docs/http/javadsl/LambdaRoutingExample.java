...
public class LambdaRoutingExample extends HttpApp {
    ...
    public static Future<RouteResult> multiplyAsync(final RequestContext ctx, final String x, final int y) {
    ...
    public Route createRoute() {
        return
            route(
                ...
                path("add").route(
                    handleWith(x, y, this::multiply)
                ),
                path("subtract").route(
                    handleWith(x, y, this::subtract)
                ),
                // matches paths like this: /multiply/{x}/{y}
                path("multiply", xSegment, ySegment).route(
                    // bind handler by reflection
                    handleWith(xSegment, ySegment, LambdaRoutingExample::multiply)
                ),
                path("multiplyAsync", xSegment, ySegment).route(
                    // bind async handler by reflection
                    handleWith(xSegment, ySegment, LambdaRoutingExample::multiplyAsync)
                )
            );
    }
