...
public class IntroSpec {
  ...
  public static class ChatRoom {
    ...
    public static Behavior<Command> mutableChatRoom3_2() { return Actor.mutable(MutableChatRoom3::new); } // WITH CONTEXT, Same as above API, jsut "modern" look ;-) ;-)
    public static Behavior<Command> mutableChatRoom3_noContext() { return Actor.mutable(MutableChatRoom3::new); } // WITH CONTEXT, Same as above API, jsut "modern" look ;-) ;-)
    public static Behavior<Command> mutableChatRoom3Mini() { 
      return Actor.mutable(ctx -> (message -> Actor.same())); // Whoa! Since target typing to interface, the shorthand is the same API (Actor.create)? 
    } 
    
    
    public static Behavior<Command> functionalThenMutable() { 
      return Actor.stateful((ctx, command) -> { /*...*/ return new MutableChatRoom3(ctx); }); 
    } 
    ...
    public static class NoContext extends Actor.MutableBehavior<Command> {
      final List<ActorRef<SessionEvent>> sessions = new ArrayList<ActorRef<SessionEvent>>();
