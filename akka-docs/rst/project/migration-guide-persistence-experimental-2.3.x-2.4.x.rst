...
================================================
``PersistentActor`` is now deprecated and replaced by ``PersistentActor`` which provides the same (and more) API.
Migrating to ``2.4.x`` is as simple as changing all your classes to extending  ``PersistentActor``.

Replace all classes like:

    class DeprecatedProcessor extends PersistentActor { /*...*/ }

To extend ``PersistentActor``:

    class NewPersistentProcessor extends PersistentProcessor { /*...*/ }
