  ...
  object `The extensions subsystem` {
    ...
    def `07 fail the system if a library-extension cannot be loaded`(): Unit =
      intercept[RuntimeException] {
        withEmptyActorSystem(
          "ExtensionsSpec07",
          Some(ConfigFactory.parseString("""akka.library-extensions += "akka.typed.FailingToLoadExtension$"""))) { _ ⇒ () }
      }
    ...
    def `08 fail the system if a library-extension cannot be loaded`(): Unit =
      intercept[RuntimeException] {
        withEmptyActorSystem(
          "ExtensionsSpec08",
          Some(ConfigFactory.parseString("""akka.library-extensions += "akka.typed.MissingExtension"""))) { _ ⇒ () }
      }
