...
import akka.http.scaladsl.model.{ ContentType, ContentTypeRange, ContentTypes, MediaTypes }
...
abstract class FramingWithContentType extends Framing {
  ...
  def matches(ct: akka.http.javadsl.model.ContentType): Boolean
  def supported: ContentTypeRange
}
object FramingWithContentType {
  def apply(framing: Flow[ByteString, ByteString, Any], contentType: ContentType, moreContentTypes: ContentType*): FramingWithContentType =
    contentType.charsetOption match {
      case Some(charset) ⇒ apply(framing, ContentTypeRange(contentType.mediaType, charset))
      case _             ⇒ apply(framing, ContentTypeRange.apply(contentType.mediaType))
    }
  def apply(framing: Flow[ByteString, ByteString, Any], contentTypeRange: ContentTypeRange) =
    new FramingWithContentType {
      override def flow: Flow[ByteString, ByteString, Unit] = framing.mapMaterializedValue(_ ⇒ ())
      override def matches(ct: model.ContentType): Boolean = contentTypeRange.matches(ct)
      override def supported: ContentTypeRange = contentTypeRange
    }
}
...
trait JsonEntityFramingSupport {
  ...
  implicit object JsonFraming extends FramingWithContentType { // TODO find the max object size
    ...
    override def matches(ct: model.ContentType): Boolean = ct.mediaType() == MediaTypes.`application/json`
    override def supported: ContentTypeRange = ContentTypes.`application/json`
  }
}
